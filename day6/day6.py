# -*- coding: utf-8 -*-
"""
Created on Mon Dec  5 23:58:57 2022
...
for ii in range(4, len(packet)):
    start = np.unique(list(packet[ii-4:ii]))
    if len(start) == 4:
        print(ii)
        break
...
"""
for ii in range(14, len(packet)):
    start = np.unique(list(packet[ii-14:ii]))
    if len(start) == 14:
        print(ii)
        break
