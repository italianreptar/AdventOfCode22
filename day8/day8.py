# -*- coding: utf-8 -*-
"""
Created on Wed Dec  7 23:55:58 2022
...
(It isn't visible from the right or bottom since other trees of height 5 are
 in the way.)
...
for row, col in product(range(rows), range(cols)):
    grid[row][col] = int(data[row][col])

mask = np.zeros(grid.shape, dtype=bool)

# Goes right across the rows
for row in range(rows):
    ...
    for col in range(cols):
        if grid[row][col] > max_val:
            ...
            mask[row][col] = True
            # print(grid[row][col], row, col, sep=" ")
...
# # Goes left across the rows
for row in range(rows):
    max_val = -1
    for col in range(cols-1, 0-1, -1):
        if grid[row][col] > max_val:
            max_val = grid[row][col]
            mask[row][col] = True
            # print(grid[row][col], row, col, sep=" ")
...
(If a tree is right on the edge, at least one of its viewing distances will be
 zero.)
...
for r, c in product(range(rows), range(cols)):
    ...
    for row in range(r+1,rows):
        if grid[row][col] < max_val:
            ...
            # print("A", grid[row][col], row, col, sep=" ")

        if grid[row][col] >= max_val:
            score[0] += 1
            # print("B", grid[row][col], row, col, sep=" ")
            break

    row = r
    max_val = grid[r][c]
    # Rightwards across row
    ...
    row = r
    max_val = grid[r][c]
    # Leftwards across rows
    for col in range(c-1,0-1,-1):
        if grid[row][col] < max_val:
            score[3] += 1
            min_val = grid[row][col]
            # print("A", grid[row][col], row, col, sep=" ")
    ...
    scores[r][c] = np.product(score)

max_r = np.argmax(np.max(scores, axis=1))
max_c = np.argmax(np.max(scores, axis=0))
print(scores[max_r][max_c])
print(80*"*")
