# -*- coding: utf-8 -*-
"""
Created on Sat Dec  3 13:10:28 2022
...
rucksacks = [(item[:len(item)//2], item[len(item)//2:]) for item in data]
...
groups = [tuple(data[i:(i+3)]) for i in range(0, len(data), 3)]
score = 0
for group in groups:
    left, center, right = group
    for item in left:
        if item in right and item in center:
            score += get_priority(item)
            break
print(score)
